...
// I assume that num_ctas is inferable from cluster_dims. TODO(manany): Verify.
        ...
        for (int block_n : BLOCK_SIZES) {
          ...
          for (int block_k : BLOCK_SIZES) {
            ...
            for (int split_k : SPLIT_K) {
              ...
              // Hopper-specific autotuning.
              if (enable_hopper_optimizations) {
                for (bool enable_ws : WARP_SPECIALIZATION) {
                  for (TritonGemmConfig::ClusterDims cluster_dims :
                       CLUSTER_DIMS) {
                    auto config = TritonGemmConfig(
                        block_m, block_n, block_k, split_k, num_stages,
                        num_warps, InferNumCtas(cluster_dims), cluster_dims,
                        enable_ws);
                    configs.push_back(std::move(config));
                  }
                }
              } else {
                auto config = TritonGemmConfig(block_m, block_n, block_k,
                                               split_k, num_stages, num_warps);
                configs.push_back(std::move(config));
              }
            }
          }
