...
SymbolicTile::SymbolicTile(absl::Span<int64_t const> target_shape,
  ...
  for (int dim = 0; dim < num_target_dims; ++dim) {
    ...
    AffineExpr offset = mlir::getAffineDimExpr(2 * dim + 1, mlir_context);

    exprs.push_back(mlir::getAffineBinaryOpExpr(
        mlir::AffineExprKind::Add,
        mlir::getAffineBinaryOpExpr(mlir::AffineExprKind::Mul, stride,
                                    tile_size),
        offset));
  ...
  this->affine_map_ = affine_map;
  this->sizes_ =
      std::vector<std::optional<int64_t>>(num_target_dims, std::nullopt);
  this->max_sizes_ =
      std::vector<int64_t>(target_shape.begin(), target_shape.end());
  this->max_strides_and_offsets_ = max_strides_and_offsets;
}
...
std::optional<SymbolicTile> SymbolicTile::TryPropagateTileThroughIndexingMap(
  ...
  AffineMap composed_map = indexing_map.affine_map.compose(this->affine_map_);
  ...
  new_max_sizes.reserve(this->max_sizes_.size());
  new_sizes.reserve(this->sizes_.size());

  // The symbols in 'composed_map' should be ordered such that the symbols
  // declared in 'indexing_map.affine_map' precede those defined in
  // this->affine_map_.
  ...
  for (const auto [msize, max_size] :
       llvm::zip(this->sizes_, this->max_sizes_)) {
    int64_t symbol_bound = msize.has_value() ? msize.value() : max_size;
    simplifier.SetInclusiveBounds(getAffineSymbolExpr(symbol_index++, ctx),
                                  /*lower=*/0, /*upper=*/symbol_bound - 1);
  }
  absl::c_copy(this->sizes_, std::back_inserter(new_sizes));
  absl::c_copy(this->max_sizes_, std::back_inserter(new_max_sizes));
  ...
  for (const auto [dim_id, max_stride_or_offset] :
       llvm::enumerate(this->max_strides_and_offsets_)) {
