...
class HloValue : public BufferValue {
 public:
  ...
  // If include_uses is true, the returned string will include `GetUses` which
  // may produce incorrect uses since the uses should be only initialized after
  // `SetPositions` is called.
  std::string ToString(int indent, bool include_uses = true) const;
