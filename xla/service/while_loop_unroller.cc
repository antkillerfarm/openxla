...
// A utility function that decides whether a loop is unrollable or not. It
// receives the list of dependee while instructions (other instructions have
// explicit control dependencies to while loops in this list).
static std::optional<WhileLoopConfig> IsLoopUnrollable(
    HloInstruction* while_op, std::vector<HloInstruction*> while_dependees) {
  CHECK(while_op->opcode() == HloOpcode::kWhile);

  // TODO(b/300668690): Check control dependencies to the while instruction
...
absl::flat_hash_map<HloInstruction*, WhileLoopConfig> GetUnrollableLoops(
  ...
  for (HloInstruction* instr : all_while_ops) {
    std::optional<WhileLoopConfig> config =
        IsLoopUnrollable(instr, while_dependees);
    if (config.has_value()) {
...
StatusOr<bool> Unroll(HloInstruction* while_op, int64_t unroll_factor,
  ...
  // In case the config is not given, we construct it by calling
  // IsLoopUnrollable.
  if (!config.has_value()) {
    std::vector<HloInstruction*> while_dependees;
    ...
    config = IsLoopUnrollable(while_op, while_dependees);
    if (!config.has_value()) {
      return false;
    }
  }
  ...
  if (config.value().trip_count > kUnrollTripCountThreshold) {
    return FailedPrecondition(
        "%s", absl::StrCat("Cannot unroll while loop. The tip count is greater "
                           "than the threshold: ",
                           config.value().trip_count, " vs ",
                           kUnrollTripCountThreshold));
  }
  ...
  for (int64_t i = config.value().init;
       i < config.value().trip_count + config.value().init; ++i) {
    CHECK(OverflowSafeAdd(i, (int64_t)1).has_value());

    HloComputation* unrolled_body = module->AddEmbeddedComputation(
        UnrollSingleIterationOfTrivialLoop(while_op,
                                           config.value().induction_var_idx, i)
            .value());
  ...
  TF_RETURN_IF_ERROR(AsyncOpCanonicalizer().Run(module).status());
...
StatusOr<bool> WhileLoopUnroller::Run(
  ...
  for (auto& [while_op, config] : unrollable_while_ops) {
    TF_ASSIGN_OR_RETURN(bool unrolled,
                        Unroll(while_op, unroll_factor_, config));
    changed |= unrolled;
  }
