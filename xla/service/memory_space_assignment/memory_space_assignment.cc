...
StatusOr<std::optional<int64_t>> GetPrefetchByEagerness(
  ...
  return static_cast<std::optional<int64_t>>(
      earliest_prefetch_time * (1.0 - prefetch_eagerness) +
      latest_prefetch_time * (prefetch_eagerness));
}
...
StatusOr<std::optional<int64_t>> GetOverriddenPreferredPrefetchTime(
    const OverridePreferredPrefetches& override_preferred_prefetches,
    int64_t operand_size, const HloUse& hlo_use,
  ...
  for (const auto& override : override_preferred_prefetches.overrides()) {
    if (!DoesOperandMatchFilter(override.hlo_operand_filter(), operand_size,
                                hlo_use)) {
      continue;
    }
    LOG(INFO) << "Config " << absl::StrCat(override)
    ...
    auto override_options = override.override_options();
    switch (override_options.options_case()) {
      case OverridePreferredPrefetchOptions::kPrefetchEagerness:
        return GetPrefetchByEagerness(override_options.prefetch_eagerness(),
                                      earliest_prefetch_time,
                                      latest_prefetch_time);
      case OverridePreferredPrefetchOptions::kAfterInstructionName:
        return GetPrefetchTimeAfterInstruction(
            override_options.after_instruction_name(), instruction_schedule);
      case OverridePreferredPrefetchOptions::kBeforeInstructionName:
        return GetPrefetchTimeBeforeInstruction(
            override_options.before_instruction_name(), instruction_schedule);
      case OverridePreferredPrefetchOptions::OPTIONS_NOT_SET:
        break;
    }
  }
...
bool DoesResultMatchFilter(const HloResultFilter& filter,
...
int64_t GetBufferIntervalPriority(
  ...
  for (int64_t i = 0; i < msa_sort_order_overrides.overrides_size(); ++i) {
    ...
    if (!DoesResultMatchFilter(override.hlo_result_filter(),
                               buffer_interval.buffer->index(),
                               buffer_interval.buffer->instruction())) {
      continue;
    }
    LOG(INFO) << "Override Sort Order Config " << absl::StrCat(override)
              << " matches "
      ...
      if (hlo_use.instruction->opcode() != HloOpcode::kBitcast ||
        ...
        auto overridden_preferred_prefetch_time =
            GetOverriddenPreferredPrefetchTime(
                options_.override_preferred_prefetches, allocation_value.size(),
...
MemoryBoundednessBufferIntervalComparator::GetTuple(
  ...
  int64_t priority = GetBufferIntervalPriority(
